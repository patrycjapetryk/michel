...
const backTxtPosition = (event, display) => {
	if (display) {
...
const contactDisplay = event => {
	...
	if (contact == false) {
		...
		contactArticle.addEventListener("mouseover", () =>
			linkDescription.classList.remove("active")
		);
		contactArticle.addEventListener("mouseout", () =>
...
const showGallery = () => {
	...
	document.body.removeEventListener("click", showGallery);
...
window.addEventListener("load", () => {
	if (!queryWidth.matches) {
		...
		window.addEventListener("mousemove", event => backTxtPosition(event, true));
		window.addEventListener("mouseout", event => backTxtPosition(event));

		document.body.addEventListener("click", showGallery);
	} else {
		showGallery();
	}
